...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Turn()
    {
        ...
        // Only flip the gun's X-axis scale, preserving its original Y and Z scales
    ...
    private IEnumerator Shoot()
    {
        ...
        if (Parent.name.Contains("ShotgunEnemy"))
        {
            ...
            for (int i = 0; i < pelletsPerShot; i++)
            {
                ...
                if (rb != null)
                {
                    // Add a random spread to each bullet
                    float angle = Random.Range(-spreadAngle / 2, spreadAngle / 2);
                    Vector2 spreadDirection = Quaternion.Euler(0, 0, angle) * directionToPlayer;

                    // Set the velocity of the bullet
                    rb.velocity = spreadDirection * bulletSpeed;
                }

                // Play the shoot sound for each pellet
                shootSound.Play();

                // Set the shooter for bullet damage recognition
    ...
    private bool IsPlatformAbove()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(transform.position, direction, 50f, groundLayer);  // Adjust the distance as needed
